...
import (
	...
	"reflect"
	"strings"
	originaltexttemplate "text/template"
	"unicode"
	"unicode/utf8"
)
...
var builtins = originaltexttemplate.FuncMap{
	"and":      and,
	"call":     call,
	"html":     HTMLEscaper,
	"index":    index,
...
func createValueFuncs(funcMap originaltexttemplate.FuncMap) map[string]reflect.Value {
...
func addValueFuncs(out map[string]reflect.Value, in originaltexttemplate.FuncMap) {
...
func addFuncs(out, in originaltexttemplate.FuncMap) {
	for name, fn := range in {
		out[name] = fn
	}
}
